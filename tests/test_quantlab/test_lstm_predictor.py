...
class TestLSTMPredictor:
    ...
    def test_preprocess_data(self, sample_time_series_data):
        ...
        X, y = predictor.preprocess_data(
            sample_time_series_data, target_column="target_return", fit_scaler=True
        )
    ...
    def test_train_basic(self, sample_time_series_data):
        ...
        predictor = LSTMPredictor(
            model_type="lstm", sequence_length=10, hidden_size=32, num_layers=1
        )
        ...
        history = predictor.train(
            X[:300], y[:300], X[300:], y[300:], epochs=2, verbose=False
        )
...
class TestLSTMDataPipeline:
    ...
    def test_prepare_data(self, sample_time_series_data):
        ...
        df_clean, feature_cols = pipeline.prepare_data(
            sample_time_series_data, target_column="target_return"
        )
    ...
    def test_split_data(self, sample_time_series_data):
        ...
        pipeline = LSTMDataPipeline(
            train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, min_samples=100
        )
    ...
    def test_get_arrays(self, sample_time_series_data):
        ...
        df_clean, feature_cols = pipeline.prepare_data(sample_time_series_data)
        X, y = pipeline.get_arrays(
            df_clean, feature_cols, target_column="target_return"
        )
    ...
    def test_prepare_lstm_data_convenience(self, sample_time_series_data):
        ...
        (X_train, y_train), (X_val, y_val), (X_test, y_test), feature_cols = (
            prepare_lstm_data(sample_time_series_data, target_column="target_return")
        )
...
class TestLSTMIntegration:
    ...
    def test_full_pipeline(self, sample_time_series_data):
        ...
        # Prepare data
        (X_train, y_train), (X_val, y_val), (X_test, y_test), feature_cols = (
            prepare_lstm_data(sample_time_series_data)
        )
    ...
    def test_gru_vs_lstm(self, sample_time_series_data):
        ...
        lstm_predictor = LSTMPredictor(
            model_type="lstm", sequence_length=10, hidden_size=32
        )
        X_scaled, _ = lstm_predictor.preprocess_data(
            pd.DataFrame(X_train[:200], columns=feature_cols).assign(
                target_return=y_train[:200]
            ),
            fit_scaler=True,
        )
        lstm_history = lstm_predictor.train(
            X_scaled, y_train[:200], epochs=2, verbose=False
        )

        # GRU
        gru_predictor = LSTMPredictor(
            model_type="gru", sequence_length=10, hidden_size=32
        )
        ...
        gru_history = gru_predictor.train(
            X_scaled, y_train[:200], epochs=2, verbose=False
        )
