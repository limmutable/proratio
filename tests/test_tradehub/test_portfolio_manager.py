...
def test_equal_allocation():
    ...
    pm = PortfolioManager(total_capital=10000, allocation_method="equal", use_ai_regime_detection=False)
...
def test_calculate_strategy_suitability():
    ...
    suitability = pm.calculate_strategy_suitability("AIEnhancedStrategy", trending_up_regime)
...
def test_market_adaptive_allocation(sample_dataframe):
    ...
    assert allocations["AIEnhancedStrategy"].weight > allocations["MeanReversion"].weight
...
def test_get_portfolio_summary():
    ...
    pm.register_strategy(MockMeanReversionStrategy(name="Strategy2"), initial_weight=0.4)
