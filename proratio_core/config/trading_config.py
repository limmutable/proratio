...
class PositionSizingConfig:
    ...
    ai_confidence_multiplier_min: float = 0.8  # Stake multiplier at min confidence (60%)
    ai_confidence_multiplier_max: float = 1.2  # Stake multiplier at max confidence (100%)
...
class StrategyConfig:
    ...
    # Strategy selection
    strategy_name: str = 'AIEnhancedStrategy'  # SimpleTestStrategy or AIEnhancedStrategy
...
class TradingConfig:
    ...
    def validate(self) -> List[str]:
        ...
        # Risk validation
        if self.risk.max_loss_per_trade_pct <= 0 or self.risk.max_loss_per_trade_pct > 10:
            errors.append("max_loss_per_trade_pct must be between 0 and 10")

        if self.risk.max_total_drawdown_pct <= 0 or self.risk.max_total_drawdown_pct > 50:
        ...
        if self.position_sizing.ai_confidence_min < 0 or self.position_sizing.ai_confidence_min > 1:
    ...
    def print_summary(self) -> None:
        ...
        print(f"  Position Size: {self.risk.min_position_size_pct}-{self.risk.max_position_size_pct}%")

        print("\n💰 POSITION SIZING")
        print(f"  Method: {self.position_sizing.method}")
        print(f"  Base Risk: {self.position_sizing.base_risk_pct}%")
        print(f"  AI Confidence Range: {self.position_sizing.ai_confidence_min:.0%} - 100%")
        print(f"  Stake Multiplier: {self.position_sizing.ai_confidence_multiplier_min}x - {self.position_sizing.ai_confidence_multiplier_max}x")

        print("\n🤖 AI CONFIGURATION")
        print(f"  Weights: ChatGPT {self.ai.chatgpt_weight:.0%}, Claude {self.ai.claude_weight:.0%}, Gemini {self.ai.gemini_weight:.0%}")
