...
class WeeklyReportGenerator:
    ...
    def calculate_metrics(self, trades: List[Dict]) -> Dict:
        ...
        return {
            ...
            'avg_profit_pct': sum(t['close_profit'] for t in closed_trades) / len(closed_trades),
    ...
    def generate_report(
        ...
        report.append("=" * 80)
        report.append(f"Report Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        ...
        report.append(f"Total Profit:          ${metrics['total_profit_usdt']:.2f} USDT")
        ...
        report.append(f"Avg Trade Duration:    {metrics['avg_trade_duration_hours']:.1f} hours")
        report.append("")
        ...
        if trades:
            ...
            for trade in trades:
                ...
                emoji = "🔓" if trade['is_open'] else ("✅" if trade['close_profit'] and trade['close_profit'] > 0 else "❌")

                report.append(f"{emoji} Trade #{trade['id']} - {trade['pair']} [{status}]")
                ...
                if not trade['is_open']:
                    ...
                    report.append(f"  Profit:  {profit_sign}${trade['close_profit_abs']:.2f} USDT ({profit_sign}{trade['close_profit']:.2f}%)")
        ...
        report.append("")
        report.append(f"Active Providers: 2/3 (66.7%)")
        report.append(f"Consensus Threshold: 60%")
        report.append(f"Dynamic Reweighting: Enabled (Claude 60% → 100% when needed)")
        report.append("")
        ...
        if metrics['closed_trades'] > 0:
            ...
            report.append("⚠️  Not enough data for comparison (need at least 1 closed trade)")
        ...
        if metrics['closed_trades'] == 0:
            report.append("• No trades executed - AI confidence threshold (60%) may be too high")
            report.append("• Consider lowering threshold to 50-55% for more opportunities")
