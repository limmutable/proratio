...
def check_openai_models():
    ...
    try:
        ...
        if 'quota' in str(e).lower():
            print("  ⚠️  API quota exceeded - add credits at https://platform.openai.com/billing")
        return None
...
if __name__ == "__main__":
    ...
    if results['openai']:
        ...
        print(f"OpenAI:    ✗ Not available (quota/key issue)")

    if results['anthropic']:
        print(f"Anthropic: {results['anthropic']}")
    else:
        print(f"Anthropic: ✗ Not available")
    ...
    if any(results.values()):
        ...
        if results['openai']:
            print(f"   proratio_signals/llm_providers/chatgpt.py")
            print(f"   → Change default model to: {results['openai']}\n")

        if results['anthropic']:
            print(f"   proratio_signals/llm_providers/claude.py")
            print(f"   → Change default model to: {results['anthropic']}\n")

        if results['gemini']:
            print(f"   proratio_signals/llm_providers/gemini.py")
