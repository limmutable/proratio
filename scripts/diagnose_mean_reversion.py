...
def main():
    ...
    for pair in pairs:
        ...
        df['bb_lower'], df['bb_middle'], df['bb_upper'] = calculate_bollinger_bands(df['close'], period=20, std=2.0)
        ...
        print(f"   Price vs BB: min={(df['close'] - df['bb_lower']).min():.2f}, max={(df['close'] - df['bb_upper']).max():.2f}")
        ...
        if len(long_opportunities) > 0:
            ...
            for idx, row in long_opportunities.head(5).iterrows():
                print(f"   - {idx}: RSI={row['rsi']:.2f}, Price={row['close']:.2f}, BB_lower={row['bb_lower']:.2f}")
        ...
        if len(short_opportunities) > 0:
            ...
            for idx, row in short_opportunities.head(5).iterrows():
                print(f"   - {idx}: RSI={row['rsi']:.2f}, Price={row['close']:.2f}, BB_upper={row['bb_upper']:.2f}")
