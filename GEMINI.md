...
This project uses SpecKit to structure development. The workflow is as follows:
1.  **New Feature**: Start a new feature with `create-new-feature.sh`. This creates a branch and a `specs` directory.
2.  **Specification**: Define the feature in `spec.md`.
3.  **Planning**: Use `setup-plan.sh` to create `plan.md`. Detail the implementation plan and technology stack here.
4.  **AI Context Update**: Run `update-agent-context.sh` to parse the plan and update the AI agent context files (`OPENCODE.md`, `GEMINI.md`, etc.).
5.  **Implementation**: The AI agent implements the feature according to the plan.
...
- **CCXT Docs**: https://docs.cc-xt.com/
