...
class FeatureEngineer:
    ...
    def add_technical_indicators(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Added {len([c for c in df.columns if c not in dataframe.columns])} technical indicators")
    ...
    def add_volatility_features(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        # Rolling volatility
        df['volatility_7d'] = df['close'].pct_change().rolling(7*24).std() * 100  # 7 days for 1h candles
        df['volatility_1d'] = df['close'].pct_change().rolling(24).std() * 100    # 1 day for 1h candles
    ...
    def add_regime_features(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'atr_pct' in df.columns and 'bb_width' in df.columns:
            df['is_volatile'] = ((df['atr_pct'] > 2.5) & (df['bb_width'] > 0.04)).astype(int)
...
def create_target_labels(dataframe: pd.DataFrame,
                        target_type: str = 'regression',
                        lookahead_periods: int = 4) -> pd.DataFrame:
    """
    ...
    if target_type == 'regression':
        ...
        df['target_return'] = ((df['close'].shift(-lookahead_periods) - df['close']) / df['close']) * 100

    elif target_type == 'classification':
        # Future direction (0=down, 1=neutral, 2=up)
        future_return = ((df['close'].shift(-lookahead_periods) - df['close']) / df['close']) * 100
